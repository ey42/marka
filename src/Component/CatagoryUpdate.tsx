    ...
    <div className="mt-4 ml-4 flex items-center justify-center border-1 rounded-md dark:border-slate-300 border-dark w-full">
      <div className="flex flex-row max-lg:flex-col max-lg:justify-center max-lg:items-center justify-between max-sm:ml-2 max-sm:justify-start max-sm:items-start">
        <form onSubmit={onsubmit} className="flex flex-col gap-5 mt-4 items-center justify-center">
          <div className="w-52 h-52 dark:bg-slate-300 bg-dark transition-all duration-200 rounded-md text-slate-300 dark:text-dark font-bold">
            <Image src={image as string} alt="catagory image" width={208} height={208} className="w-52 h-52 border-2 dark:border-slate-300 border-dark"/>
          </div>
        ...
        <div className={`max-sm:w-40 text-dark dark:text-slate-300 text-sm flex flex-col max-lg:justify-center max-lg:items-center justify-start max-sm:justify-start max-sm:items-start lg:ml-36 gap-4 max-sm:ml-0`}> <h1 className="text-center transition-all duration-200 text-dark dark:text-slate-300 font-semibold text-lg">
          {access !== undefined && access.length > 0 ?`These are list of ${access?.length! <= 1 ? "catagory" : "catagories"} in the database `: `no catagories`}</h1>
          
                  <ul className="text-sm font-semibold text-dark dark:text-slate-50 max-md:grid-rows-10 max-sm:grid-cols-1  grid grid-cols-3 grid-flow-row gap-x-10 gap-5 dark:marker:text-white list-none items-center max-sm:justify-start max-sm:items-start justify-center">
                { 
access !== undefined ? access.map((n) => (
  <li key={n} className="border-2 dark:border-slate-300 w-32 bg-slate dark:bg-dark transition-all duration-200 border-dark h-11 overflow-hidden outline-1 ring-1 rounded-md ring-green-300 text-center text-dark dark:text-slate-300 px-2 "> {n} </li>
                )
              ) : "loading..."
               } 
               </ul></div>
