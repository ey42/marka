...
access !== undefined ? access.map((n) => (
  ...
  <li  className="border-2 dark:border-slate-50 w-32 bg-slate dark:bg-dark transition-all duration-200 border-dark h-11 overflow-hidden cursor-pointer rounded-md text-center pt-2 px-2 hover:bg-black dark:hover:bg-light dark:hover:text-black hover:text-light"> {n.replace(/_/g, ' ')} </li>
  </Link>
                )
              ) : "loading..."
               } 
